...
import sys, logging, os, json, random
...
def addToInv(item,current):
    if item in rooms[current]["inventory"]:
        pass
...
def randomStart():
    ...
    if random.randint(0,1):
        if random.randint(0,1):
            pos = yList[random.randint(0,5)]+'1'
        else:
            pos = yList[random.randint(0,5)]+'6'
    else:
        if random.randint(0,1):
            pos = 'A'+str(random.randint(1,6))
        else:
            pos = 'F'+str(random.randint(1,6))
    if pos in {'A3','A4','C6','D6','E6'}:
        return randomStart()
    else:
        return pos
